...
- frontend build and typecheck work are standardized on Node `20.x`; use `frontend/.nvmrc` or an equivalent pinned Node 20 runtime locally so your environment matches CI and the frontend container build
- the normal frontend validation gate is still `npm run build`
- the phased helpers in `frontend/package.json` are diagnostic-only: use `npm run build:compile` followed by `npm run build:generate` when you need to isolate compile-vs-generate failures in the same workspace state
